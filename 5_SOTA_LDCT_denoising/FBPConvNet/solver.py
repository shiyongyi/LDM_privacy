...
from networks import FBPCONVNet
...
class Solver(object):
    def __init__(self, args, data_loader):
        ...
        self.FBPConv = FBPCONVNet()
        if (self.multi_gpu) and (torch.cuda.device_count() > 1):
            print('Use {} GPUs'.format(torch.cuda.device_count()))
            self.FBPConv = nn.DataParallel(self.FBPConv)
        self.FBPConv.to(self.device)

        self.lr = args.lr
        self.criterion = nn.MSELoss()
        self.optimizer = optim.Adam(self.FBPConv.parameters(), self.lr)


    def save_model(self, iter_):
        f = os.path.join(self.save_path, 'FBPConv_{}iter.ckpt'.format(iter_))
        torch.save(self.FBPConv.state_dict(), f)
    ...
    def load_model(self, iter_):
        ...
        if self.multi_gpu:
            ...
            for k, v in torch.load(f):
                n = k[7:]
                state_d[n] = v
            self.FBPConv.load_state_dict(state_d)
        else:
            self.FBPConv.load_state_dict(torch.load(f))
    ...
    def test(self):
        del self.FBPConv
        # load
        self.FBPConv = FBPCONVNet().to(self.device)
