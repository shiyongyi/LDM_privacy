...
from recon.models import FistaNet
...
class net(pl.LightningModule):
    def __init__(self, args):
        ...
        if self.is_vis_show:
            self.vis = Visualizer(env='FistaNet')

    def forward(self, x, p):
        out, loss_layers_sym, loss_st = self.model(x, p)
        return out, loss_layers_sym, loss_st

    def training_step(self, batch, batch_idx):
        x, p, y = batch
        out, loss_layers_sym, loss_st = self(x, p)
        loss_discrepancy = F.mse_loss(out, y)
        loss_constraint = torch.mean(loss_layers_sym ** 2)
        sparsity_constraint = torch.mean(loss_st.abs())
